...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(rightChild < last){
        ...
        if(compare(dyGet(heap, leftChild), dyGet(heap, rightChild)) == -1){
            ...
            indexSmallest = rightChild;
        }

        if(compare(dyGet(heap, position), dyGet(heap, indexSmallest)) == 1){
            dySwap(heap, position, indexSmallest);
            adjustHeap(heap, last, indexSmallest, compare);
        }
    }
 ...
 * @param heap  array ith elements in any order.
 * @param compare  pointer to compare function.
 */
